...
**Kazanma sahnesi.** Top çıktıktan 0,5 sn sonra oyun alanının ortasında üç yıldız yuvası belirir; kazanılan yıldızlar 120 ms arayla dolar. Altında sonuç ("İyi açılış · 4,3 sn") ve varsa bir üst yıldıza kalan pay ya da rekor. Toplam süre değişmez (1,4 sn). Dokunmak sahneyi atlatır ve sonraki bölüm hemen gelir; atlatan çift dokunuşun ikincisi yeni bölümün ilk halkasını kilitlemesin diye yeni bölümün ilk 0,2 sn'sinde dokunuşlar yok sayılır. Dolu/boş ayrımı şekille (★/☆) taşınır, renkle değil: amber açık zeminde 1,97:1'dir. Sahne `aria-hidden`'dır; aynı sonuç alt çubuktaki canlı bölgede sözle duyurulur. "Hareketi azalt" açıksa yıldızlar animasyonsuz belirir. Yayın öncesi incelemede "ödül var, sahnesi yok" bulunmuştu: sonuç yalnız alt çubukta 15 px metindi. Sahnenin en altında **toplam birikim** görünür ("Toplam 71 / 108 yıldız"): toplam yıldız eskiden yalnız Ayarlar'daydı. Üst çubuğa konmadı; 320 pikselde orası zaten punto kısılarak sığıyor. Her 50 bölümde bir, bölüm ilk kez bitirilince bu satır **kilometre taşına** döner ("50 bölüm geride kaldı!") ve alt çubukta da sözle duyurulur.

**Günün bölümü.** Duraklatma ekranında "Günün bölümü: 37" düğmesi: yerel tarihten türetilen, oyuncunun **açtığı** bölümler arasından seçilen bir bölüm (havuz 10'luk basamaklara yuvarlanır ki gün içinde ilerledikçe değişmesin). En az 10 bölüm açılınca görünür. Bitirilince üst üste gün sayısı (seri) yazılır ve kazanma sahnesinde "Günün bölümü tamam · 3 günlük seri" çıkar; aynı gün ikinci kez sayılmaz, bir gün atlanınca seri 1'e döner. Kilitli bölüm açmaz, ilerlemeyi atlatmaz, tabloya dokunmaz. Seri ayrı bir anahtarda (`donence:gunluk:v1`) tutulur: tablo değişip rekorlar silinse bile kalır. Kaynak `game/gunluk.ts`. Yayın öncesi incelemede "geri dönme sebebi yok" bulunmuştu.
...
- **Kırmızı yalnız yanılan halkaya kadar.** Kayıpta kırmızı dolgu ve kesik kontur merkezden kaybettiren kilidin halkasına kadar çizilir, bütün çıkış yolu boyunca değil. Dilim dışa doğru genişlediği için dış halkalar hizasında geniş görünüyor ve oyuncuya "top buradan geçerdi" dedirtiyordu (proje sahibinin gözlemi). Süre dolmasında daralan bir halka olmadığı için eski çizim kalır.
- **Top kural boyutunda çizilir.** Kural topun gerçek boyutuna 3° emniyet payı ekler (`NEED`); top gerçek boyutunda çizilince kıl payı kayıpta gözle sığıyordu (360 px ekranda açıklık ~18,6 px, top 15,8 px). Çizilen yarıçap artık eşikten türetilir (`TOP_CIZIM = inner · sin(NEED_PASS/2)`, ~%21 büyük): eşikte açıklığa tam oturur, altında sığmadığı görülür. Kural ve tablo değişmez.
...
- **iOS açılış görselleri:** iOS, ana ekrandan açılan web uygulamasında manifest'in açılış rengini kullanmaz; `apple-touch-startup-image` yoksa kısa bir beyaz ekran gösterir. 14 cihaz ölçüsü için görsel `npm run icons` ile üretilir (koyu zemin, ortada simge) ve `index.html`'de medya sorgusuyla bağlanır; bir test liste, dosya ölçüleri ve etiketleri eşler. Servis çalışanının önbelleğine alınmaz (~350 kB, iOS kendisi saklar).
- **Paylaşım önizlemesi:** `index.html` Open Graph ve X etiketleri taşır; görsel (`og.png`, 1200×630, yazısız) `npm run icons` ile oyunun geometrisinden üretilir. `og:image` ve `og:url` mutlak adres ister: **alan adı değişirse ikisi güncellenmeli.**
- **Çevrimdışı:** tüm derleme çıktısı servis çalışanıyla önbelleğe alınır; yazı tipi de dahil, çünkü Fredoka projeye gömülüdür (bkz. 7. bölüm) ve hiçbir dış alan adına istek yapılmaz. Oyun ilk bölüm kazanıldığında tarayıcıdan kalıcı depolama ister (`navigator.storage.persist`): sekmede oynayanın kaydı depolama baskısında ya da Safari'de 7 gün kullanılmayınca silinebiliyordu.
- **Güncelleme:** yeni sürüm sessizce indirilir ve **yalnız bölüm sınırında** uygulanır (bölüm kazanılınca ya da kaybedilip yeniden başlarken); oyuncuya sorulmaz, turu kesilmez. Duraklatmadan dönüş eskiden de güvenli an sayılıyordu, ama yenileme bölümü baştan başlatıyor ve duraklatma ekranının "halkalar tam durduğun yerde bekliyor" sözünü bozuyordu. Tarayıcı servis çalışanını yalnız gezinmede denetler ve ana ekrana eklenmiş oyun arka plandan dönünce sayfa yeniden yüklenmez; bu yüzden oyun yeni sürümü saatte bir ve her öne gelişte kendisi sorar. Sormasaydı oyuncu yeni sürümü günlerce almayabilirdi — tablo değiştiğinde bu, eski tabloda rekor biriktirmek demek.
- **Güvenlik başlıkları** (`vercel.json`, her yol): sıkı CSP (`default-src 'self'`, satır içi betik ve stil yok, `frame-ancestors 'none'`), `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, `Permissions-Policy` (kamera, mikrofon, konum, ödeme, USB kapalı), `X-Frame-Options: DENY`. Oyun hiçbir dış kaynak kullanmadığı için politika hiçbir şeyi kırmaz; gizlilik sayfasının stili bu yüzden satır içi değil `gizlilik.css` dosyasındadır. Doğrulama: aynı başlıklarla sunulan derleme görünmez Chrome'da açıldı, oyunda ve gizlilik sayfasında 0 ihlal (karşı sınama: bilerek satır içi betik ve stil taşıyan sayfada 2 ihlal yakalandı).
